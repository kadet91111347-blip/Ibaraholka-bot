...
def _next_id() -> int:
    """Generate unique id (microsec timestamp + random). Works on SQLite and Postgres."""
    import random
    return int(time.time() * 1_000_000) + random.randint(0, 999)
        ...
        with db_cursor() as conn:
            conn.execute(
                """INSERT INTO conversations
                   (id, user_id, username, user_message, bot_response, intent, response_variant, created)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                (_next_id(), user_id, username, user_msg[:500], bot_resp[:1000], intent, variant, int(time.time()))
            )
...
async def deals_create(request: Request, user: Dict = Depends(get_user)):
    ...
    with db_cursor() as conn:
        ...
        if payment_method == "ton":
            ...
            conn.execute(
                "CREATE TABLE IF NOT EXISTS ton_payments ("
                "id BIGINT, listing_id TEXT, tier TEXT, "
...
# deploy-trigger 1789763000 v82: rate limit + improved health + Sentry + openapi tags + Docker + GitHub Actions: payment modal opens even if /listings fails (loadListings wrapped in try/catch)
